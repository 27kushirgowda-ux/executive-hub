...
export default function MentorDashboard({ dark }) {
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      if (user) {
        // 🎯 Real-time Mentor Profile
        unsubMentor = onSnapshot(doc(db, "users", user.uid), (snap) => {
          if (snap.exists()) setMentorData(snap.data());
        });

        // 🎯 Real-time Intern Mapping (Querying for THIS mentor's ID)
        // ⚠️ IF THIS RETURNS 0: Check the Browser Console for an Index Link!
        const q = query(
          collection(db, "users"), 
          where("role", "==", "Intern"), 
        ...
        unsubInterns = onSnapshot(q, (snap) => {
          ...
          // Filter out any potential garbage data and sort
          setInterns(list.filter(i => i.name).sort((a, b) => a.name.localeCompare(b.name)));
          setLoading(false);
        }, (err) => {
  ...
  return (
    <div className={`w-full min-h-screen p-4 md:p-8 space-y-10 animate-in fade-in duration-1000 ${styles.text}`}>
      ...
      <div className={`${styles.card} p-10 md:p-14 rounded-[3.5rem] border flex flex-col md:flex-row justify-between items-center gap-8`}>
        ...
        <div className="flex items-center gap-4 relative z-20">
           <div className={`px-6 py-3 rounded-full border border-white/10 flex items-center gap-3 ${dark ? 'bg-white/5' : 'bg-black/5'}`}>
             <ShieldCheck size={18} className={styles.accent}/>
             <span className="text-[10px] font-black uppercase tracking-widest">Master Clearance Active</span>
           </div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-20">
        <div className={`${styles.card} p-10 rounded-[3rem] border flex flex-col justify-center`}>
          <div className={styles.innerShine}></div>
          <div className="flex items-center gap-4 mb-6 opacity-40 relative z-20">
            <IdCard size={20}/>
            <h3 className="text-[11px] font-black uppercase tracking-[0.3em] italic">Professional Identifier</h3>
          </div>
          <div className={`p-6 rounded-2xl flex items-center justify-between border relative z-20 ${dark ? 'bg-black/40 border-white/5 shadow-inner' : 'bg-white/80 border-slate-100 shadow-sm'}`}>
            <code className={`text-xs font-mono font-black truncate mr-6 ${styles.accent}`}>{auth.currentUser?.uid}</code>
            <button onClick={copyId} className={`${styles.accent} hover:scale-110 transition shrink-0`}>
              {copied ? <CheckCircle size={24}/> : <Copy size={24}/>}
            </button>
          </div>
        </div>

        <div className={`${styles.card} p-10 rounded-[3rem] border flex items-center justify-between`}>
          <div className={styles.innerShine}></div>
          <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center relative z-20 ${dark ? 'bg-amber-500/10 text-amber-500' : 'bg-purple-600 text-white shadow-2xl'}`}>
            <Users size={36} />
          </div>
          <div className="text-right relative z-20">
            ...
            <p className={`text-[11px] font-black uppercase tracking-[0.4em] ${styles.sub} mt-2`}>Mapped Interns</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8 relative z-20">
        <div className="relative group max-w-xl">
          ...
          <input 
            ...
            className={`w-full pl-16 pr-8 py-6 rounded-[2.5rem] border outline-none font-black text-[11px] tracking-widest uppercase transition-all ${dark ? 'bg-white/5 border-white/10 text-white placeholder:text-white/10' : 'bg-white/60 border-white shadow-xl text-slate-900'}`} 
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {interns
            ...
            .map(intern => (
            <div key={intern.id} className={`${styles.card} p-10 rounded-[3.5rem] border transition-transform hover:-translate-y-2`}>
               <div className={styles.innerShine}></div>
               <div className="flex items-center gap-5 mb-10 relative z-20">
                  <div className={`w-16 h-16 rounded-[1.8rem] flex items-center justify-center text-black font-black text-2xl shadow-xl ${dark ? 'bg-amber-500 shadow-amber-500/20' : 'bg-gradient-to-br from-pink-500 to-purple-700 text-white'}`}>
               ...
               <div className="space-y-6 relative z-20 mb-10">
                 <div className="flex justify-between items-end">
                    <div>
                       <p className={`text-[10px] font-black uppercase tracking-widest ${styles.sub} mb-1`}>Total Tenure</p>
                       <div className="flex items-center gap-2">
                          <Timer size={14} className={styles.accent}/>
                          <span className="text-sm font-black italic uppercase">{intern.internshipDuration || 90} Day Protocol</span>
                       </div>
                    </div>
               ...
               <button 
                 ...
                 className={`w-full py-5 rounded-[1.8rem] flex items-center justify-center gap-4 text-[11px] font-black uppercase tracking-[0.3em] transition-all relative z-20 ${
                   dark ? 'bg-white/5 border border-white/5 text-white hover:bg-amber-500 hover:text-black' : 'bg-slate-100 text-[#1e1b4b] hover:bg-[#1e1b4b] hover:text-white'
                 }`}
               >
                 Review Ledger <ArrowRight size={16} />
               </button>
            </div>
          ))}
        </div>
        ...
        {interns.length === 0 && !loading && (
          <div className={`${styles.card} py-32 text-center rounded-[4rem] border border-white/5`}>
            ...
            <div className="relative z-20 opacity-20">
               ...
               <p className="font-black uppercase tracking-[0.4em] text-xs">No Interns Linked to Hub ID</p>
               <p className="text-[10px] mt-4 font-mono">{auth.currentUser?.uid}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-center pt-10 pb-12">
         <div className={`${styles.card} px-12 py-6 rounded-full border border-white/[0.08] flex items-center gap-6 shadow-2xl transition-all hover:-translate-y-1`}>
            ...
            <p className="text-[10px] font-black uppercase italic tracking-[0.3em] opacity-40 text-center leading-none">
              Verified Supervisor Session Active. Real-time Sync Enabled.
            </p>
         </div>
      </div>
    </div>
